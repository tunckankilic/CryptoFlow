...
🆕 Yeni Promptlar (14-16)
## PROMPT 14: Firebase Auth Paketi
packages/auth paketi oluştur - Firebase Authentication:
...
1. lib/domain/entities/app_user.dart:
@immutable
...
enum AuthProvider { google, apple, email, anonymous }

2. lib/domain/repositories/auth_repository.dart:
...
4. lib/data/datasources/firebase_auth_datasource.dart:
...
5. lib/data/repositories/auth_repository_impl.dart

PRESENTATION LAYER:

6. lib/presentation/bloc/auth_bloc.dart:
// Events
...
7. lib/presentation/pages/login_page.dart:
- Logo + Title
- Google Sign In Button
- Apple Sign In Button (iOS only)
- Skip (Anonymous) option

8. lib/presentation/pages/profile_page.dart:
- User info display
- Sign out button
- Account deletion option

9. lib/presentation/widgets/:
- SocialLoginButton
- UserAvatar
- ProfileHeader

## PROMPT 15: Cloud Sync + Teknik İndikatörler
İki özellik ekle: Cloud Sync ve Teknik İndikatörler

BÖLÜM 1: CLOUD SYNC SERVICE

1. packages/core/lib/services/cloud_sync_service.dart:
class CloudSyncService {
  final FirebaseFirestore _firestore;
  final AuthRepository _authRepository;
  
  CollectionReference _userDoc(String uid) => 
    _firestore.collection('users').doc(uid).collection('data');
  
  Future<void> syncPortfolio(List<Holding> holdings) async {
    final user = await _authRepository.getCurrentUser();
    if (user == null || user.isAnonymous) return;
    
    await _userDoc(user.uid).doc('portfolio').set({
      'holdings': holdings.map((h) => h.toJson()).toList(),
      'updatedAt': FieldValue.serverTimestamp(),
    });
  }
  
  Future<List<Holding>> fetchPortfolio() async {...}
...
2. Auto-sync on changes in BLoCs

---

BÖLÜM 2: TEKNİK İNDİKATÖRLER

3. packages/market/lib/domain/services/technical_indicators.dart:
class TechnicalIndicators {
  ...
  /// EMA - Exponential Moving Average (period: 9, 21, 50, 200)
  static List<double?> calculateEMA(List<Candle> candles, {int period = 21}) {
    // EMA calculation with multiplier
  }
...
enum IndicatorType { rsi, ema9, ema21, ema50, sma20, macd }

5. Update candle_bloc.dart:
class ToggleIndicator extends CandleEvent {
  final IndicatorType indicator;
}

6. Update candle_chart.dart:
- Main chart with EMA overlay
- RSI panel (separate, with 70/30 lines)
- MACD panel (separate, with histogram)

7. IndicatorSelector widget:
FilterChip for each indicator type

## PROMPT 16: CI/CD Pipeline (GitHub Actions)
...
[![codecov](https://codecov.io/gh/tunckankilic/cryptoflow/branch/main/graph/badge.svg)]
...
1. **core** - Network, WebSocket client, error handling, formatters, cloud sync
2. **design_system** - Kripto UI kit (PriceText, CandleChart, OrderBookView)
3. **auth** - Firebase Auth (Google, Apple, Anonymous)
4. **market** - Binance API, ticker/candle/orderbook streams, teknik indikatörler
5. **portfolio** - Holding takibi, transaction management, P&L
6. **alerts** - Fiyat alarmları
7. **watchlist** - Takip listesi
8. **settings** - Kullanıcı ayarları
...
2. TickerListBloc - Market listesi + WebSocket
3. TickerDetailBloc - Coin detay
4. CandleBloc - Candles + Indicators
5. PortfolioBloc - Holdings + P&L
6. AlertBloc - Price alerts
7. WatchlistBloc - Favorites
...
## ⏳ Mevcut Durum
[Buraya kaldığın yeri yaz]

## 🎯 Sonraki Adım
[Buraya bir sonraki yapılacak işi yaz]

---

Yukarıdaki context ile devam ediyorum.
...
Gelişmiş Özellikler (v1.5)

 PROMPT 14: Firebase Auth
 PROMPT 15: Cloud Sync + Teknik İndikatörler
 PROMPT 16: CI/CD Pipeline

📊 Prompt Özet Tablosu
PromptİçerikTahmini Süre1Proje Kurulumu1-2 saat2Core Package2-3 saat3Design System2-3 saat4Market Domain2-3 saat5Market Data3-4 saat6Market Presentation4-5 saat7Portfolio Package3-4 saat8Alerts Package2-3 saat9Watchlist Package2-3 saat10DI & Navigation2-3 saat11Testing3-4 saat12Performance2-3 saat13Release Hazırlık2-3 saat14Firebase Auth3-4 saat15Cloud Sync + İndikatörler3-4 saat16CI/CD Pipeline1-2 saat
Toplam: ~40-50 saat