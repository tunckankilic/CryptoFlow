...
# Read Apple Sign-In private key from SSM at plan/apply time.
data "aws_ssm_parameter" "apple_private_key" {
  name            = var.apple_private_key_ssm_parameter
  with_decryption = true
}

# Federated identity provider attached to the existing user pool.
# Project is iOS-only (CV scope) — Apple Sign In only, Google omitted.
